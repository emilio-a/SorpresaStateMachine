...
class machinemulator:
    def __init__(self, top=None):
        ...
        top.title("FSMachinemulator V0.12")
        ...
        self.varcombobox_puerto = tk.StringVar()
        self.progreso = tk.IntVar()      #vble para la barra de progreso
        
        _style_code()
        self.Memory_tlf = ttk.Labelframe(self.top)
        self.Memory_tlf.place(relx=0.281, rely=0.114, relheight=0.478, relwidth=0.23)
        ...
        self.Clock_tlf.place(relx=0.282, rely=0.616, relheight=0.119, relwidth=0.449)
        ...
        self.FF_tlf.place(relx=0.544, rely=0.114, relheight=0.478, relwidth=0.193)
        ...
        self.sub_menu.add_command(compound='left',label='Guardar...', command=guarda_archivo)
        
        self.TLabelframe1 = ttk.Labelframe(self.top)
        self.TLabelframe1.place(relx=0.105, rely=0.748, relheight=0.203, relwidth=0.719)
        self.TLabelframe1.configure(relief='')
        self.TLabelframe1.configure(text='''Comm Port''')
        self.TCombobox_puerto = ttk.Combobox(self.TLabelframe1)
        self.TCombobox_puerto.place(relx=0.032, rely=0.208, relheight=0.2, relwidth=0.405, bordermode='ignore')
        self.TCombobox_puerto.configure(textvariable=self.varcombobox_puerto)
        self.TCombobox_puerto.configure(takefocus="")
        self.TButton_leeRam = ttk.Button(self.TLabelframe1)
        self.TButton_leeRam.place(relx=0.512, rely=0.24, height=30, width=93, bordermode='ignore')
        self.TButton_leeRam.configure(takefocus="")
        self.TButton_leeRam.configure(text='''Lee RAM''')
        self.TButton_leeRam.configure(compound='left')
        self.TCheckbutton_verifica = ttk.Checkbutton(self.TLabelframe1)
        self.TCheckbutton_verifica.place(relx=0.78, rely=0.56, relwidth=0.156, relheight=0.0, height=23, bordermode='ignore')
        self.TCheckbutton_verifica.configure(variable=self.verifica)
        self.TCheckbutton_verifica.configure(takefocus="")
        self.TCheckbutton_verifica.configure(text='''Verifica''')
        self.TCheckbutton_verifica.configure(compound='left')
        self.TButton_escribeRam = ttk.Button(self.TLabelframe1)
        self.TButton_escribeRam.place(relx=0.512, rely=0.56, height=30, width=93, bordermode='ignore')
        self.TButton_escribeRam.configure(takefocus="")
        self.TButton_escribeRam.configure(text='''Escribe RAM''')
        self.TButton_escribeRam.configure(compound='left')
        self.TProgressbar_transfer = ttk.Progressbar(self.TLabelframe1, variable=self.progreso)
        self.TProgressbar_transfer.place(relx=0.029, rely=0.688, relwidth=0.415, relheight=0.0, height=19, bordermode='ignore')
        self.TProgressbar_transfer.configure(length="170")
        
        self.mi_timer=RepeatedTimer(1,evento_clock)      #creo un timer repetitivo de 1 seg de duracion

#--------------- funcion para leer el hex desde el arduino -----------------
def click_leeRam(evento):
        if(_w1.TCombobox_puerto.get() == ""): #string.IsNullOrEmpty(TCombobox_port.Text)):
                messagebox.showinfo(message="Chequear puerto COM", title="Puerto!!")
        else:
                try:
                        s=serial.Serial(port=_w1.TCombobox_puerto.get(), baudrate=9600, rtscts = False, dsrdtr = False)     # abro puerto seleccionado en el combobox

                        s.flush()                                       # limpio los buffers
                        s.timeout=1.5     #un segundo de timeout
                        for veces in range(1,3):
                                s.write(b"eco\n")
                                respuesta=s.read(size=4)        #eco<LF>
                        s.flush()
                        if(intelhex.minaddr()!=None):                   # si existe HEX en memoria
                                _w1.progreso.set(0)                     # barra de progreso en 0
                                for direccion in range(intelhex.minaddr(), intelhex.maxaddr()):   # voy secuenciando la direccion
                                        _w1.progreso.set(direccion/intelhex.maxaddr()*100)        # aumento lA barra de progreso
                                        s.write(b"r:")
                                        s.write((hex(direccion).replace('0x','')).encode())       # mando por el puerto serie la direccion
                                        s.write(bytes(b"\n"))   #mando un <LF>, queda armado el paquete dir,dat<LF>                                              # 
                                       
                                        root.update_idletasks() #actualizo las tareas de fondo mienstras esta transmitiendo
                                        s.timeout=1.5     #un segundo de timeout
                                        respuesta=s.read(size=3)
                                        respuesta=respuesta.decode()
                                        intelhex[direccion]=int(respuesta)
                                        if not respuesta:
                                                messagebox.showinfo(message="No hubo respuesta", title="Sin respuesta!!")
                                                break
                                        #intelhex[direccion]=respuesta   #ojo chequear esto

                        _w1.progreso.set(0)     #barra de progreso en 0 cuando termino
                        s.close()               # cierro el puerto (esto provoca un reset via DTR? revisar)
                except (OSError, serial.SerialException):
                        pass
        return
#-------------- Funcion para escribir en RAM via puerto serial -------------
def click_escribeRam(evento):
        if(_w1.TCombobox_puerto.get() == ""): #string.IsNullOrEmpty(TCombobox_port.Text)):
                messagebox.showinfo(message="Chequear puerto COM", title="Puerto!!")
        else:
                try:
                        s=serial.Serial(port=_w1.TCombobox_puerto.get(), baudrate=9600)     # abro puerto seleccionado en el combobox
                        s.flush()                                       # limpio los buffers
                        if(intelhex.minaddr()!=None):                   # si existe HEX en memoria
                                _w1.progreso.set(0)                     # barra de progreso en 0
                                for direccion in range(intelhex.minaddr(), intelhex.maxaddr()):   # voy secuenciando la direccion
                                        ...
                                        s.write((hex(direccion).replace('0x','')).encode())       # mando por el puerto serie la direccion
                                        s.write(b":")                                             # 
                                        s.write((hex(intelhex[direccion]).replace('0x','')).encode()) # mando el dato
                                        s.write(bytes(b"\n"))   #mando un <LF>, queda armado el paquete dir,dat<LF> 
                                        root.update_idletasks() #actualizo las tareas de fondo mienstras esta transmitiendo
                                        s.timeout=1     #un segundo de timeout
                                        respuesta=s.read(size=3)
                                        if not respuesta:
                                                messagebox.showinfo(message="No hubo respuesta", title="Sin respuesta!!")
                                                break
...
# --------------- manejo del combobox del puerto ------------------
def click_TCombobox_puerto(evento):
        ports = glob.glob('/dev/tty[A-Za-z]*')
        portlist=['']
        for port in ports:
            try:
                s = serial.Serial(port, baudrate=9600, rtscts = False, dsrdtr = False)
                s.close()
                portlist.append(port)
            except (OSError, serial.SerialException):
                pass
        _w1.TCombobox_puerto["values"]= portlist
        if len(portlist) >1:
                _w1.TCombobox_puerto.current(1)
        return

def seleccion_TCombo_puerto(evento):
        print("Se selecciono combobox puertos")
...
def carga_archivo():
        ...
        click_reset_button(0)    #simulo un reset button
        return

def guarda_archivo():
        archivo=fd.asksaveasfile(title="Guardar Archivo", filetypes=(('Intel Hex Files', '*.hex'),('Todos', '*.*')), mode='w',defaultextension=".hex")
        intelhex.write_hex_file(archivo)
        return
...
def click_run_stop_button(evento): 
        global corriendo        
        if(intelhex.minaddr()==None):
                messagebox.showinfo(message="No hay HEX cargado!", title="Archivo!")
        else:
                if corriendo==0:#
...
def main(*args):
    try:
        ...
        _w1.TCombobox_puerto.bind('<Button-1>', click_TCombobox_puerto)
        _w1.TCombobox_puerto.bind('<<ComboboxSelected>>', seleccion_TCombo_puerto)
        _w1.TButton_escribeRam.bind('<Button-1>', click_escribeRam)
        _w1.TButton_leeRam.bind('<Button-1>', click_leeRam)
        
        #----- configuro acciones iniciales ------
        _w1.clock_led.configure(style="LedAmOff.TFrame") # apago led de clock
        _w1.ff_clk.configure(background=_bgcolor)   # apago label de clock
        clock=0
        ...
        #------- detecto los puertos y defino el combobox -------
        ports = glob.glob('/dev/tty[A-Za-z]*')          # obtengo lista de puertos del SO 
        portlist=['']                                   # creo una string vacia
        for port in ports:
                try:
                        s = serial.Serial(port, baudrate=9600, rtscts = False, dsrdtr = False)  #hago un probing de los puertos para ver cual se puede abrir
                        s.close()
                        portlist.append(port)
                except (OSError, serial.SerialException):
                        pass
